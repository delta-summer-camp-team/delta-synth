...
fn build_audio_modules(synthstate: Arc<SynthState>) -> Vec<Arc<Mutex<dyn AudioModule>>> {
  ...
  let osc1 = Oscillator::new(1, 440.0, 44100.0,  synthstate.clone());
  let osc2 = Oscillator::new(2, 440.0, 44100.0,  synthstate.clone());
  let osc3 = Oscillator::new(3, 440.0, 44100.0,  synthstate.clone());
