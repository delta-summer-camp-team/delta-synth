...
impl AdvGate {
  ...
  fn update_envelop(&mut self) {
    ...
    let attack = self.synth_state.gate_attack.load(Ordering::Relaxed)as f32 / 127.0;
    let decay = self.synth_state.gate_decay.load(Ordering::Relaxed)as f32 / 127.0; 
    let sustain = self.synth_state.gate_sustain.load(Ordering::Relaxed)as f32 / 127.0;
    let release = self.synth_state.gate_release.load(Ordering::Relaxed)as f32 / 127.0;
