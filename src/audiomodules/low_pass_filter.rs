...
impl LowPassFilter {
  ...
  fn filter(&mut self, x: f32) -> f32 {
    ...
    let y: f32 = ((2 * Q + dt * self.cutoff) * self.buffer1 + Q * self.buffer2
      - Q * self.cutoff * self.cutoff * dt * dt * x)
      / (Q + dt * self.cutoff + self.cutoff * self.cutoff); //discrete solution solution of a differential equation, don't ask
                                                            //shift state
